...
| **股票筛选** | **智能选股** | **从中证1000/800/200/沪深300/中证500中，基于7条核心交易理念筛选优质股票，每日推送1-2只候选** |
=======
| 回测 | AI 回测验证 | 自动评估历史分析准确率，支持按股票与分析日期查看“AI 预测 vs 次日实际（1 日窗口）”和准确率 |
>>>>>>> dffa29c5fdb5a273a62c6d059944905654da1b1e
...
### 股票筛选
```
🎯 2026-03-20 中证1000 每日优选股票

基于以下核心交易理念筛选：
- ✅ 趋势交易：MA5 > MA10 > MA20 多头排列
- ✅ 严进策略：乖离率 < 5%（强势股可放宽至 8%）
- ✅ 效率优先：换手率 1-5%
- ✅ 量价配合：量比 0.8-3
...
```bash
# 从中证1000筛选2只优质股票
python main.py --screen --index 000852 --top-n 2

# 从沪深300筛选3只
python main.py --screen --index 000300 --top-n 3
```

**GitHub Actions**：
已创建独立的 `股票筛选` 工作流，每日北京时间 17:30 自动执行。可手动触发并选择指数和返回数量。
